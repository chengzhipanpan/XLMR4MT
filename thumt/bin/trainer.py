...
from transformers.models.xlm_roberta.modeling_xlm_roberta import XLMRobertaForMaskedLM
...
def main(args):
    ...
    while True:
        for features in dataset:
            ...
            # start_t = time.time()
            features = data.to_translation_features(features, sep_id)
            ...
            counter += 1
            t = time.time()
            # print("Data Time Gap \t", t - start_t)

            translate_loss, denoised_loss = train_fn(features, mode)
            loss = translate_loss + denoised_loss
            ...
            print("step = %d, loss = %.3f, length_loss = %.3f, (%.3f sec)" %
                (step, float(translate_loss), float(denoised_loss), t))

            total_loss += float(translate_loss.item())
            if mode == "sundae":
                total_length_loss += float(denoised_loss.item())
            total_step += 1
            total_time += t
            if total_step == 100:
