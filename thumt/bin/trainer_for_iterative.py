...
from transformers.models.xlm_roberta.modeling_xlm_roberta import XLMRobertaForCausalLM, XLMRobertaModel, XLMRobertaForMaskedLM
...
def print_variables(model, pattern, log=True):
    ...
    for name in sorted(list(weights)):
        if weights[name].requires_grad==True:
            v = weights[name]
...
def main(args):
    ...
    print("Adding Sepcial Tokens...", flush=True)
    ...
    if args.half:
        xlmr_model = xlmr_model.cuda().half()
    ...
    def train_fn(inputs, mode):
        features, labels, denoised_target = inputs

        loss = model(features, labels, denoised_target, mode)
        return loss
    ...
    while True:
        for features in dataset:
            ...
            t = time.time()
            features = data.to_translation_features_v5(features, sep_id)
            ...
            translate_loss, length_loss = train_fn(features, mode)
            denoised_loss = 0
            loss = translate_loss + length_loss
            ...
            summary.scalar("translate_loss", translate_loss, step, write_every_n_steps=1)
            # summary.scalar("denoised_loss", denoised_loss, step, write_every_n_steps=1)
            summary.scalar("global_step/sec", t, step)

            print("step = %d, loss = %.3f, length_loss = %.3f, (%.3f sec)" %
                (step, float(translate_loss), float(length_loss), t))

            total_loss += float(translate_loss.item())
            if mode == "sundae":
                total_length_loss += float(length_loss.item())
            total_step += 1
            total_time += t
            if total_step == 100:
                print("step = %d, trnaslate_loss = %.3f, denoised_loss = %.3f (%.3f sec)" %
                    (step, float(total_loss) / 100, float(total_length_loss) / 100, total_time))
                total_step = 0
                total_time = 0
                total_loss = 0
                total_length_loss = 0
